package com.arseeenyyy.github.server.db;
...
import static com.arseeenyyy.github.server.app.Server.LOGGER;

public class DataBaseManager implements InDataBaseManager {
    private Connection connection;
    private final String COLLECTION = "SELECT dragon.id, dragon.name, coordinates.x, coordinates.y, dragon.creationdate, dragon.age, dragon.description, dragon.wingspan, dragon.color, dragon_cave.number_of_treasures, users.user_id " +
            "FROM dragon " +
            "INNER JOIN users on dragon.user_id = users.user_id " +
            "INNER JOIN coordinates ON dragon.coordinates_id = coordinates.coord_id " +
            "INNER JOIN dragon_cave ON dragon.cave_id = dragon_cave.cave_id where users.user_id = ? ";
    private final String ALL_ELEMENTS = "SELECT dragon.id, dragon.name, coordinates.x, coordinates.y, dragon.creationdate, dragon.age, dragon.description, dragon.wingspan, dragon.color, dragon_cave.number_of_treasures, users.user_id " +
            "FROM dragon " +
            " INNER JOIN users ON dragon.user_id = users.user_id " +
            " INNER JOIN coordinates ON dragon.coordinates_id = coordinates.coord_id " +
    ...
    private final String ALIEN_COLLECTION = "SELECT dragon.id, dragon.name, coordinates.x, coordinates.y, dragon.creationdate, dragon.age, dragon.description, dragon.wingspan, dragon.color, dragon_cave.number_of_treasures " +
            ...
            "INNER JOIN users on dragon.user_id = users.user_id" +
            "INNER JOIN coordinates ON dragon.coordinates_id = coordinates.coord_id " +
            "INNER JOIN dragon_cave ON dragon.cave_id = dragon_cave.ve_id where user_id <> ?";
    private final String REMOVE_BY_ID = "delete from dragon where id = ? and user_id = ?";
    private final String REMOVE_BY_WINGSPAN = "DELETE FROM dragon where wingspan = ? and user_id = ?";
    ...
    private final String UPDATE_DRAGON = "update dragon set name = ?, coordinates_id = ?, age = ?, description = ?, wingspan = ?, color = ?, cave_id = ? where id = ? and user_id = ?";

    public Connection connectToDatabase() {
        Properties info = new Properties();
        try {
            info.load(new FileInputStream("/home/meemo/Documents/Программирование/lab8/lab8/lab-server/src/main/java/com/arseeenyyy/github/server/db/db.cfg"));
    ...
    public ArrayList<Dragon> getAllElementsFromDataBase() {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(ALL_ELEMENTS);
            ...
            while (resultSet.next()) {
                ...
                Dragon dragon = new Dragon(id, name, coordinates, toLocalDateTime(creationDate), age, description, wingspan, color, new DragonCave(numberOfTreasures), userId);
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(ALIEN_COLLECTION)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    Color color = Color.valueOf(resultSet.getString("color"));
                    long numberOfTreasures = resultSet.getLong("number_of_treasures");
                    int alien_user_id = resultSet.getInt("user_id");

                    Dragon dragon = new Dragon(id, name, coordinates, toLocalDateTime(creationDate), age, description, wingspan, color, new DragonCave(numberOfTreasures), alien_user_id);
    ...
    public boolean removeDragonByWingspan(double wingspan, int userId) {
        connection = connectToDatabase();
    ...
    public boolean getUser(Request request) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(GET_USER)) {
            ...
            String password = Encryptor.sha384(user.getPassword());
    ...
    public boolean deleteAllByUserId(int userId) {
        ...
        try {
            ...
            try (PreparedStatement deleteDragonsStmt = connection.prepareStatement(DELETE_DRAGONS_BY_USER_ID);
                 PreparedStatement deleteCoordinatesStmt = connection.prepareStatement(DELETE_COORDINATES_BY_USER_ID);
    ...
    public void updateDragon(Request request) {
        connection = connectToDatabase();
        Integer coordinates_id = null;
        try (PreparedStatement coordinatesStatement = connection.prepareStatement(INSERT_NEW_COORDINATES)) {
            coordinatesStatement.setInt(1, request.getDragon().getCoordinates().getX());
            coordinatesStatement.setFloat(2, request.getDragon().getCoordinates().getY());
            ResultSet rs = coordinatesStatement.executeQuery();
            if (rs.next()) coordinates_id = rs.getInt("coord_id");
            else throw new SQLException();

        }catch (SQLException exception) {LOGGER.error("cordid", exception);}
        Integer cave_id = null;
        try (PreparedStatement caveStatement = connection.prepareStatement(INSERT_NEW_CAVE)) {
            caveStatement.setLong(1, request.getDragon().getCave().getNumberOfTreasures());
            ResultSet rs = caveStatement.executeQuery();
            if (rs.next()) cave_id = rs.getInt("cave_id");
            else throw new SQLException();
        }catch (SQLException exception) {LOGGER.error("caveID", exception);}

        try (PreparedStatement updateStatement = connection.prepareStatement(UPDATE_DRAGON)) {
            Dragon dragon = request.getDragon();
            updateStatement.setString(1, dragon.getName());
            updateStatement.setInt(2, coordinates_id);
            updateStatement.setLong(3, dragon.getAge());
            updateStatement.setString(4, dragon.getDescription());
            updateStatement.setDouble(5, dragon.getWingspan());
            PGobject colorObject = new PGobject();
            colorObject.setType("color");
            colorObject.setValue(dragon.getColor().toString());
            updateStatement.setObject(6, colorObject);
            updateStatement.setInt(7, cave_id);
            updateStatement.setInt(8, dragon.getId());
            updateStatement.setInt(9, request.getUser().getId());
            updateStatement.executeUpdate();
        }catch (SQLException exception) {
            LOGGER.error("dragoninsertion", exception);
        }
        finally {
            try {
                connection.close();
            }catch (SQLException exception) {
                LOGGER.error("closing exception", exception);
            }
        }
